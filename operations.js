...
function table(headers,data){if(!data.length)return '<div class="empty">当前范围暂无记录。可切换日期、门店或筛选条件。</div>';return '<div class="table-wrap" tabindex="0" role="region" aria-label="数据表，可滚动查看"><table><thead><tr>'+headers.map(x=>'<th scope="col">'+esc(x)+'</th>').join('')+'</tr></thead><tbody>'+data.map(r=>'<tr>'+r.map(v=>'<td>'+v+'</td>').join('')+'</tr>').join('')+'</tbody></table></div>'}
